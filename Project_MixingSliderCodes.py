...
class Robot(object):
    ...
    def __init__(self, x, y):
        ...
        ##Creating Slider and Buttons
        self.button = Button(toolbar, text="QUIT", fg="black", cursor="hand2", command=root.destroy)
        self.button.pack(side=LEFT)
        self.speak = Button(toolbar, text="Click", cursor="hand2", command=self.write_speak)
        self.speak.pack(side=LEFT)
        self.start = Button(toolbar, text="Start", cursor="hand2", command=self.movement(canvas))
        self.start.pack(side=LEFT, padx=2, pady=2)
        self.slider = Scale(toolbar, variable=dist, orient=HORIZONTAL, length=300, width=15, sliderLength=10, \
                            from_=1, to=10, tickinterval=1, cursor="hand2", bg="grey")
        self.slider.pack(fill=X, padx=4, pady=4)
        self.changeSpeedButton = Button(root, text="Get Current Speed Value", cursor="hand2", command=speedSelection)
        self.changeSpeedButton.pack()
        self.label = Label()
        self.label.pack()
